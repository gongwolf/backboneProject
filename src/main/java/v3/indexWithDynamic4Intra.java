...
public class indexWithDynamic4Intra {

    final int single = 1;
    final int multiple = 2;
    public ProgramProperty prop = new ProgramProperty();
    //index data structure
    public ArrayList<Hashtable<Long, Hashtable<Long, ArrayList<double[]>>>> index = new ArrayList();  //level --> <highway node id --->{ source nod  id ==> <skyline paths > }  >
    //intra index data structure that store the distance information between nodes in each layer
    public ArrayList<Hashtable<Long, Hashtable<Long, ArrayList<double[]>>>> intra_index = new ArrayList();  //level --> <highway node id --->{ source nod  id ==> <skyline paths > }  >
    public ArrayList<Hashtable<Long, ArrayList<Long>>> nodesToHighway_index = new ArrayList(); // level --> sid --> the list of it's highway in current level
    int graphsize = 100;
    int degree = 3;
    int dimension = 3;
    ...
    public static void main(String args[]) throws CloneNotSupportedException {
        ...
        indexWithDynamic4Intra index = new indexWithDynamic4Intra();
        index.build();
        long end = System.currentTimeMillis();
        System.out.println("Total Running time " + (end - start) * 1.0 / 1000 + "  ~~~~~ ");
    }
...
//        test();
    ...
    private void writeNodesToHighwayToDisk(ArrayList<Hashtable<Long, ArrayList<Long>>> nodes_to_highway_index) {
        ...
        try {
            String sub_folder_str = "/home/gqxwolf/mydata/projectData/BackBone/indexes/busline_" + graphsize + "_" + same_t + "/nodeToHighway_index";
    ...
    private void writeToDisk(Hashtable<Long, Hashtable<Long, ArrayList<double[]>>> layer_index, int level) {
        BufferedWriter writer = null;
        try {
            String sub_folder_str = "/home/gqxwolf/mydata/projectData/BackBone/indexes/busline_" + graphsize + "_" + same_t + "/level" + level;
            File sub_folder_f = new File(sub_folder_str);
            if (sub_folder_f.exists()) {
                sub_folder_f.delete();
            }
    ...
    private void writeIntraIndexToDisk(Hashtable<Long, Hashtable<Long, ArrayList<double[]>>> layer_index, int level) {
        BufferedWriter writer = null;
        try {
            String sub_folder_str = "/home/gqxwolf/mydata/projectData/BackBone/indexes/busline_" + graphsize + "_" + same_t + "/intraLayer/level" + level;
...
//        int max = (int) (this.degree_pairs_sum * percentage);
    ...
    private void initLevel() {
        int currentLevel = 0;
        String sub_db_name = graphsize + "_" + same_t + "_Level" + currentLevel;
    ...
    private long handleUpperLevelGraph(int currentLevel, Pair<Integer, Integer> threshold_p, int threshold_t) throws CloneNotSupportedException {
        ...
        //The graph after delete the edges
...
//                boolean t_f = updatedHighwayofHnode(highway_n_id, layer_index, nodesToHighWay);
     ...
     * <p>
     * 1) But nid still could go from itself to some highway nodes, so don't need to deleted the nid's information in the nodesToHighway Information
     *
     ...
     * @return updated layer_index, nodesToHighWay, intra_layer_index
     */
    private boolean clearLayerIndex(Hashtable<Long, Hashtable<Long, ArrayList<double[]>>> layer_index, Hashtable<Long, ArrayList<Long>> nodesToHighWay, HashSet<Long> deletedNodes, Hashtable<Long, Hashtable<Long, ArrayList<double[]>>> intra_layer_index) {
        try {
            for (long nid : deletedNodes) {
                /** Clear the nodes to highway information, nid can not go to any highway node anymore, because nid is removed. **/
                if (layer_index.get(nid) != null) { // if nid has highway information
                    for (long sid : layer_index.get(nid).keySet()) { //sid is nid's highway nods.
                        if (nodesToHighWay.get(sid) != null) { //if nid is one of the highway of sid
                            int idx_source_to_nid = nodesToHighWay.get(sid).indexOf(nid); // the index of nid that is in the sid's nodesToHighWay structure
                            if (idx_source_to_nid != -1) {
                                nodesToHighWay.get(sid).remove(idx_source_to_nid);
                            }

                            if (nodesToHighWay.get(sid).isEmpty()) { //after removed, sid can not go to any highway node. remove it.
                 ...
                 * the information that nid to its highway nodes.
                 * the node nid can only be deleted once in a layer
                 **/
                if (!intra_layer_index.containsKey(nid) && layer_index.get(nid) != null) { //nid has highway nodes and no intra layer index information so far
                    Hashtable<Long, ArrayList<double[]>> intra_source_to_deleted = new Hashtable<>();
                    for (Map.Entry<Long, ArrayList<double[]>> source_of_nid : layer_index.get(nid).entrySet()) { //all the source of nid
//                        if (deletedNodes.contains(e.getKey())) {
                        intra_source_to_deleted.put(source_of_nid.getKey(), source_of_nid.getValue());
...
//                            System.out.println(" ====remove single edge at level 0  " + r);
    ...
    private void updateLayerIndex(Relationship r, Hashtable<Long, Hashtable<Long, ArrayList<double[]>>> layer_index, Hashtable<Long, ArrayList<Long>> nodesToHighWay, int type) {
        ...
        /** Remove single edge event
         the node with degree 1 needs to be deleted, the node with 1 degree is the source node **/
        if (type == single) {
            if (s_degree == 1) {
    ...
    private boolean updatedHighwayofHnode(long sid, Hashtable<Long, Hashtable<Long, ArrayList<double[]>>> layer_index, Hashtable<Long, ArrayList<Long>> nodesToHighWay) {
        ...
        try {
            if (nodesToHighWay.containsKey(sid)) {
                ...
                for (long h_id : nodesToHighWay.get(sid)) { //h_id is the node id who is the highway nodes of sid
...
//        }
    ...
    private void copyToHigherDB(int src_level, int dest_level) {
        String dest_db_name = graphsize + "_" + same_t + "_Level" + dest_level;
        String src_db_name = graphsize + "_" + same_t + "_Level" + src_level;
