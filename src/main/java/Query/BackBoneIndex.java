...
public class BackBoneIndex {
    private ArrayList<Hashtable<Long, Hashtable<Long, ArrayList<double[]>>>> index = new ArrayList();  //level --> <source --->{ highway id ==> <skyline paths > }  >
...
//                    costs[2] = new BigDecimal(String.valueOf(line.split(" ")[3])).setScale(3, BigDecimal.ROUND_HALF_UP).doubleValue();
    ...
    public HashSet<Long> getHighwayNodeAtLevel(int level, long source_node) {
        Hashtable<Long, Hashtable<Long, ArrayList<double[]>>> layer = index.get(level);
        Hashtable<Long, ArrayList<double[]>> highwaynodes = layer.get(source_node);
    ...
    public ArrayList<double[]> readHighwaysInformation(long highway_node, int level, long source_node) {
        ...
        Hashtable<Long, Hashtable<Long, ArrayList<double[]>>> layer = index.get(level);
        Hashtable<Long, ArrayList<double[]>> highway_nodes_index = layer.get(source_node);
